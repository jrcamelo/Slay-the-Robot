...
func _on_card_play_started(card_play_request: CardPlayRequest) -> void:
	...
	if card_play_request.card_data == card_data:
		return
	if not card_play_request.card_data.is_card_kind(CardData.CARD_KIND_INTRO):
		return
	if card_play_request.card_data.card_type != CardData.CARD_TYPES.ATTACK:
		return
	if card_play_request.selected_target == null or not card_play_request.selected_target.is_alive():
		return
	...
	var reaction_request: CardPlayRequest = _begin_reaction(
		...
		null,
		{"triggering_card_data": card_play_request.card_data}
	)
