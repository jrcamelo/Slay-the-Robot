...
func _get_editor_relevant_value_names() -> Array[String]:
	return [ActionValueRegistry.ARTIFACT_ID, ActionValueRegistry.CONSUMABLE_OBJECT_ID, ActionValueRegistry.CONSUMABLE_SLOT_INDEX]
		...
		if shop_data != null:
			...
			if card_data != null:
				...
				if player_money >= card_price:
					...
					action_data.append({Scripts.ACTION_ADD_MONEY: {ActionValueRegistry.MONEY_AMOUNT: -card_price}})
			...
			if artifact_id != "":
				...
				if player_money >= artifact_price:
					...
					action_data.append({Scripts.ACTION_ADD_MONEY: {ActionValueRegistry.MONEY_AMOUNT: -artifact_price}})
					action_data.append({Scripts.ACTION_ADD_ARTIFACT: {ActionValueRegistry.ARTIFACT_ID: artifact_id}})
					var generated_actions: Array[BaseAction] = ActionGenerator.create_actions(player, null, [], action_data, null)
					ActionHandler.add_actions(generated_actions)
			...
			if consumable_object_id != "":
				...
				if player_money >= consumable_price:
					if not Global.player_data.are_consumable_slots_full():
						...
						action_data.append({Scripts.ACTION_ADD_MONEY: {ActionValueRegistry.MONEY_AMOUNT: -consumable_price}})
						action_data.append({Scripts.ACTION_ADD_CONSUMABLE: {ActionValueRegistry.CONSUMABLE_OBJECT_ID: consumable_object_id, ActionValueRegistry.FILL_ALL_SLOTS: false, ActionValueRegistry.RANDOM_CONSUMABLE: false}})
						var generated_actions: Array[BaseAction] = ActionGenerator.create_actions(player, null, [], action_data, null)
						ActionHandler.add_actions(generated_actions)
